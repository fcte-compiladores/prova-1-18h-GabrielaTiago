...
class LoxTransformer(Transformer):
    ...
    # Comandos
    def var_def(self, _var_keyword, name: Var, expr: Expr | None = None):
        return VarDef(name.name, expr)

    def print_cmd(self, _print_keyword, expr):
        return Print(expr)

    def block(self, *stmts: Stmt):
        return Block(list(stmts))

    def while_loop(self, _while_keyword, cond: Expr, body: Stmt):
